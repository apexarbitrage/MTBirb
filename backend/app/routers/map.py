...
TomTom tile with the key from `.env` and stream it back. `?layer=` picks the style (`basic` road
map for the nav screen, `sat`/`hybrid` for the Trail-detail terrain map). Tiles are immutable, so we
let the browser cache them aggressively. Returns 503 when TOMTOM_API_KEY isn't set.
"""
...
async def map_tile(
    z: int, x: int, y: int, layer: str = Query("basic", pattern="^(basic|sat|hybrid)$")
) -> Response:
    try:
        data, media_type = await TomTomClient().fetch_tile(z, x, y, layer=layer)
